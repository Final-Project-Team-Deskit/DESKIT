...
**1. 다중 방송과 대용량 트래픽 처리**
   - Auto Scaling과 로드 밸런싱 <br>
       : 트래픽 증가 시, 미디어 서버를 자동 증설 <br>
       : 멀티 서버에 맞는 로드 밸런싱 전략 적용 <br>
    
   - 부하 테스트 진행 <br>
       : 현재 한계 파악 -> 동시 접속자 증가 테스트 -> 병목 지점 파악 -> 해당 부분 설정 조정 및 개선 <br>

**2. AI 성능 개선 및 적용 범위 확장**
   - RAG 기반 답변 시 고려사항 <br>
       : 정책 문서가 많아졌을 때의 검색 성능 <br>
       : 벡터 유사도 검증 <br>
...
- 고하원(팀장)
  - **원활한 협업**이 이루어져 만족스러운 결과 도출, 누군가에게 의존한 프로젝트가 아닌 **모두가 함께** 만들어낸 프로젝트
- 김도윤
  - **AI**를 활용하여 어떤 기능을 제공하면 **사용자 경험**이 개선될 지를 고민, 다양한 기술에 대한 시각을 확장하고 **설계**의 중요성을 체감
- 박용헌
  - 직접 **서버를 설계하고 배포**하면서 Docker, Github Actions 등 다양한 툴을 활용, **기술적 도전**을 해볼 수 있었던 좋은 경험
- 주장우
  - 가장 큰 수확은 **로컬과 배포 환경의 간극**을 직접 메우며 얻은 실전 경험, 접근 경로 설계 및 연결 과정에서 **인프라**에 대한 이해도를 높이는 계기

---

### 비고
- 접속 주소: `https://ssg.deskit.o-r.kr` (NCP 서버 -> 로컬 마이그레이션)
- AWS로 서버 이전 예정입니다.
