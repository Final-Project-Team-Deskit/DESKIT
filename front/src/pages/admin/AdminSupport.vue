...
const loadEvaluations = async () => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      error.value = message || '?¬ì‚¬ ëª©ë¡??ë¶ˆëŸ¬?¤ì? ëª»í–ˆ?µë‹ˆ??'
      evaluations.value = []
      return
    }
    evaluations.value = (await response.json()) as AiEvaluationSummary[]
  } catch (err) {
    error.value = err instanceof Error ? err.message : '?¬ì‚¬ ëª©ë¡??ë¶ˆëŸ¬?¤ì? ëª»í–ˆ?µë‹ˆ??'
  } finally {
    loading.value = false
  }
}
...
const loadEvaluationDetail = async (aiEvalId: number) => {
  ...
  try {
    ...
    if (!response.ok) {
      const message = await response.text()
      form.message = message || '?¬ì‚¬ ?ì„¸ ?•ë³´ë¥?ë¶ˆëŸ¬?¤ì? ëª»í–ˆ?µë‹ˆ??'
      form.hasError = true
      selected.value = null
      return
    }
    ...
    form.message = err instanceof Error ? err.message : '?¬ì‚¬ ?ì„¸ ?•ë³´ë¥?ë¶ˆëŸ¬?¤ì? ëª»í–ˆ?µë‹ˆ??'
    form.hasError = true
    selected.value = null
  } finally {
...
const finalizeEvaluation = async () => {
  ...
  try {
    ...
    if (!response.ok) {
      const message = await response.text()
      form.message = message || 'ìµœì¢… ?¬ì‚¬ ì²˜ë¦¬???¤íŒ¨?ˆìŠµ?ˆë‹¤.'
      form.hasError = true
      return
    }
    form.message = 'ìµœì¢… ?¬ì‚¬ê°€ ?„ë£Œ?˜ì—ˆ?µë‹ˆ??'
    await loadEvaluationDetail(selected.value.aiEvalId)
    await loadEvaluations()
  } catch (err) {
    form.message = err instanceof Error ? err.message : 'ìµœì¢… ?¬ì‚¬ ì²˜ë¦¬???¤íŒ¨?ˆìŠµ?ˆë‹¤.'
    form.hasError = true
  } finally {
...
const loadEscalatedChats = async () => {
  ...
  try {
    ...
    if (!response.ok) {
      const message = await response.text()
      directError.value = message || 'ë¬¸ì˜ ëª©ë¡??ë¶ˆëŸ¬?¤ì? ëª»í–ˆ?µë‹ˆ??'
      escalatedChats.value = []
      return
    }
    escalatedChats.value = (await response.json()) as DirectChatSummary[]
  } catch (err) {
    directError.value = err instanceof Error ? err.message : 'ë¬¸ì˜ ëª©ë¡??ë¶ˆëŸ¬?¤ì? ëª»í–ˆ?µë‹ˆ??'
  } finally {
...
const loadDirectChatHistory = async (chatId: number) => {
  ...
  try {
    ...
    directError.value = err instanceof Error ? err.message : 'ì±„íŒ… ?´ì—­??ë¶ˆëŸ¬?¤ì? ëª»í–ˆ?µë‹ˆ??'
  } finally {
    directLoading.value = false
  }
}
...
const acceptDirectChat = async (chatId: number) => {
  ...
  try {
    ...
    if (!response.ok) {
      const message = await response.text()
      directError.value = message || '?ë‹´ ?˜ë½???¤íŒ¨?ˆìŠµ?ˆë‹¤.'
      return
    }
    ...
    directError.value = err instanceof Error ? err.message : '?ë‹´ ?˜ë½???¤íŒ¨?ˆìŠµ?ˆë‹¤.'
  } finally {
    directLoading.value = false
  }
}
...
const closeDirectChat = async (chatId: number) => {
  ...
  try {
    ...
    if (!response.ok) {
      const message = await response.text()
      directError.value = message || '?ë‹´ ì¢…ë£Œ???¤íŒ¨?ˆìŠµ?ˆë‹¤.'
      return
    }
    ...
    directError.value = err instanceof Error ? err.message : '?ë‹´ ì¢…ë£Œ???¤íŒ¨?ˆìŠµ?ˆë‹¤.'
  } finally {
    directLoading.value = false
  }
}
...
const sendDirectMessage = async () => {
  ...
  try {
    ...
    directError.value = error instanceof Error ? error.message : 'ë©”ì‹œì§€ ?„ì†¡???¤íŒ¨?ˆìŠµ?ˆë‹¤.'
  }
}
...
<template>
  <div>
    <PageHeader eyebrow="DESKIT" title="ê³ ê°?¼í„°" />
    ...
    <section v-if="activeTab === 'sellerApproval'" class="live-section">
      <div class="live-section__head">
        <h3>?ë§¤???±ë¡ ?¹ì¸</h3>
        <p class="ds-section-sub">?ë§¤???±ë¡ ?”ì²­???•ì¸?˜ê³  ?¹ì¸?©ë‹ˆ??</p>
      </div>
      ...
      <section class="support-card ds-surface">
        <div class="support-card__head">
          <div>
            <h4>AI ?¬ì‚¬ ?´ì—­</h4>
            <p>?¬ì—…ê³„íš??AI ?¬ì‚¬ ê²°ê³¼ë¥??•ì¸?˜ê³  ìµœì¢… ?¬ì‚¬ë¥?ì§„í–‰?˜ì„¸??</p>
          </div>
        ...
        <p v-if="loading" class="state-text">?¬ì‚¬ ëª©ë¡??ë¶ˆëŸ¬?¤ëŠ” ì¤‘ì…?ˆë‹¤.</p>
        <p v-else-if="error" class="state-text error">{{ error }}</p>
        <p v-else-if="!evaluations.length" class="state-text">AI ?¬ì‚¬ ?´ì—­???†ìŠµ?ˆë‹¤.</p>
    ...
    <section v-else class="live-section">
      <div class="live-section__head">
        <h3>ë¬¸ì˜?¬í•­ ?•ì¸</h3>
        <p class="ds-section-sub">ê³ ê° ë¬¸ì˜ ?´ì—­???•ì¸?©ë‹ˆ??</p>
      </div>
      ...
      <section class="support-card ds-surface">
        ...
        <p v-if="directLoading" class="state-text">ë¬¸ì˜ ëª©ë¡??ë¶ˆëŸ¬?¤ëŠ” ì¤‘ì…?ˆë‹¤.</p>
        <p v-else-if="directError" class="state-text error">{{ directError }}</p>
        <p v-else-if="!escalatedChats.length" class="state-text">?€ê¸?ì¤‘ì¸ ë¬¸ì˜ê°€ ?†ìŠµ?ˆë‹¤.</p>
          ...
          <div class="direct-chat-panel">
            ...
            <div v-else class="direct-chat-card">
              ...
              <div class="direct-chat-messages">
                <p v-if="!directMessages.length" class="state-text">ì±„íŒ… ?´ì—­???†ìŠµ?ˆë‹¤.</p>
                <div
    ...
    <div v-if="showModal" class="evaluation-modal" role="dialog" aria-modal="true" aria-label="?¬ì‚¬ ?ì„¸">
      ...
      <div class="evaluation-modal__card ds-surface">
        <div class="evaluation-modal__head">
          <div>
            <h3>AI ?¬ì‚¬ ?ì„¸</h3>
            <p>AI ?¬ì‚¬ ê²°ê³¼ë¥??•ì¸?˜ê³  ìµœì¢… ?¬ì‚¬ë¥?ì§„í–‰?©ë‹ˆ??</p>
          </div>
          <button type="button" class="btn ghost" @click="closeModal">?«ê¸°</button>
        </div>

        <div v-if="detailLoading" class="state-text">?ì„¸ ?•ë³´ë¥?ë¶ˆëŸ¬?¤ëŠ” ì¤‘ì…?ˆë‹¤.</div>
        <div v-else-if="!selected" class="state-text error">?¬ì‚¬ ?ì„¸ ?•ë³´ë¥?ë¶ˆëŸ¬?¤ì? ëª»í–ˆ?µë‹ˆ??</div>
        <div v-else class="evaluation-detail">
          ...
          <section class="final-section">
            <div class="final-head">
              <h4>ìµœì¢… ?¬ì‚¬</h4>
              <span v-if="isFinalized" class="status-pill is-final">?„ë£Œ??/span>
            </div>
            <p class="final-desc">ìµœì¢… ?±ê¸‰ê³?ê´€ë¦¬ì ì½”ë©˜?¸ë? ?…ë ¥?˜ë©´ ?ë§¤???´ë©”?¼ë¡œ ê²°ê³¼ê°€ ?„ì†¡?©ë‹ˆ??</p>

            <div class="final-form">
              <label class="field">
                <span class="field__label">ìµœì¢… ?±ê¸‰</span>
                <select v-model="form.gradeRecommended" :disabled="isFinalized || detailLoading" class="field-input">
                  <option disabled value="">?±ê¸‰ ? íƒ</option>
              ...
              <label class="field">
                <span class="field__label">ê´€ë¦¬ì ì½”ë©˜??/span>
                <textarea
                  v-model="form.adminComment"
                  class="field-input textarea"
                  :disabled="isFinalized || detailLoading"
                  placeholder="ìµœì¢… ?¬ì‚¬ ì½”ë©˜?¸ë? ?…ë ¥?˜ì„¸??"
                ></textarea>
              </label>
              <div class="final-actions">
                ...
                >
                  {{ form.submitting ? 'ì²˜ë¦¬ ì¤?..' : 'ìµœì¢… ?¬ì‚¬ ?„ë£Œ' }}
                </button>
              </div>
            ...
            <div v-if="selected.adminEvaluation" class="final-info">
              <p>ìµœì¢… ?±ê¸‰: {{ selected.adminEvaluation.gradeRecommended }}</p>
              <p>ê´€ë¦¬ì ì½”ë©˜?? {{ selected.adminEvaluation.adminComment || '-' }}</p>
              <p>°ü¸®ÀÚ Æò°¡ÀÏ: {{ formatDateTime(selected.adminEvaluation.createdAt) }}</p>
            </div>
          </section>
        </div>
      </div>
